...
- Docker and `start-local-prototype.sh` default to `OCR_PROVIDER=tesseract` and `TRANSLATION_PROVIDER=opus_mt`.
- Mock OCR creates one synthetic text region near the top of each image with `Sample detected text` and is used for deterministic tests or explicit mock mode.
- Mock translation returns `[target_language] source text` and is used for deterministic tests or explicit mock mode.
...
- `OCR_PROVIDER=tesseract` is the default local prototype provider using native Tesseract through `pytesseract`.
...
- `TRANSLATION_PROVIDER=opus_mt` is the default local prototype provider using local pre-converted CTranslate2 OPUS-MT int8 models.
